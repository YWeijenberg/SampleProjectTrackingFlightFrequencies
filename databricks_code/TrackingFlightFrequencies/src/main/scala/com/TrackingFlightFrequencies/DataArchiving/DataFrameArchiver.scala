...
object DataFrameArchiver extends SparkSessionProvider {
  def writeDataFrameToBlob(dataFrame: DataFrame,
    ...
    val outputPath =
      s"wasb://${containerName}@${storageAccountName}.blob.core.windows.net/" +
      s"${outputDirectory}" // Specify your output directory path

    spark.conf.set(s"fs.azure.account.auth.type.${storageAccountName}.blob.core.windows.net", "SAS")
    spark.conf.set(s"fs.azure.sas.token.provider.type.${storageAccountName}.blob.core.windows.net", "org.apache.hadoop.fs.azurebfs.sas.FixedSASTokenProvider")
    spark.conf.set(s"fs.azure.sas.fixed.token.${storageAccountName}.blob.core.windows.net", sasToken)
